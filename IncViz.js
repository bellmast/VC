    ...
    for(i=0; i < orgNumber; i++) {
        for(h=i+1; h < orgNumber; h++) {
            ...
            if(i2==3) {
                ...
                startingPosX = orgXcoords[i2]+((orgRadii[i2])*Math.sin(((-orgCountBot[i2]+4)*3*Math.PI)/6))
                startingPosY = midY+((orgRadii[i2])*Math.cos(((-orgCountBot[i2]+4)*3*Math.PI)/6))
            }
            endingPosX = orgXcoords[h2]+((orgRadii[h2])*Math.sin(((-orgCountBot[h2]+4)*3*Math.PI)/6))
            endingPosY = midY+((orgRadii[h2])*Math.cos(((-orgCountBot[h2]+4)*3*Math.PI)/6)) //gotta make this backwards
            orgCountBot[h2] += 1
